...
export class RemixSyncComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.apiService
      ...
      .then((data) => {
        ...
        this.playlists.items = data.items.filter((playlist) =>
          playlist.description?.match(this.originalIdRegex)
        );
      })
  ...
  onActionClick(playlist: SpotifyApi.PlaylistObjectSimplified) {
    ...
    const fullOrignalId = playlist.description?.match(
      this.originalIdRegex
    ) as string[];
    this.originalPlaylistId = fullOrignalId[0]
      .replace('{', '')
      .replace('}', '');
    this.apiService
      .getRemixedPlaylistInformation(this.originalPlaylistId)
      .then((data) => {
        this.versions = data;
